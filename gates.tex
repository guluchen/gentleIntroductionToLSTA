...
\ol{there seems to be a bug in c) --- the result should be $[0~1]$}
  }
\label{and:gate:fig}
\end{figure}
%
...
\ol{the "2" in the right-hand top corner should be "1"}
  }
\label{qgates:fig} 
\end{figure}
In a similar manner to classical gates, we will represent quantum gate behaviors by matrices.
%
Quantum gates are {\it reversible}: if we run the gate and then run the inverse of the gate, then we get the identity matrix.\ol{we get the same \emph{state}?}
%
...
\begin{figure}[ht] 
    ...
    \ol{in the right-hand side, the 000 and 001 branches should be swapped, and also the 110 and 111 branches}
    }
    \label{apply:not:fig}
\end{figure}
